# дз №20 (logging-1)
* созданы новые контейнеры микросервисов приложения
* создан образ контейнера fluentd
* создана конфигурация doker-compose для сервисов логгирования
* изменена конфигурация fluent для разбора неструктурированных сообщений журнала 
* сделаны запросы для поиска сообщений из журнала в kibana
* в конфигурацию docker-compose добавлен контейнер zipkin
* изменены описания сервисов в docker-compose для поддержки zipkin
* проверена работоспособность zipkin 

## второе задание со ( * )

* в методе post_app.find_post() в файле post_app.py на строке 167 стоит инструкция time.sleep(3), которая на первый взгляд бессмысленна 
